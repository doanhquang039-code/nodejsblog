-- Migration Fix: Safe Role Update
-- Date: 2024-04-27
-- Description: Cập nhật role an toàn, không mất dữ liệu

-- ============================================
-- 1. BACKUP & UPDATE EXISTING USERS
-- ============================================

-- Tạm thời thêm cột role_new
ALTER TABLE users ADD COLUMN IF NOT EXISTS role_new VARCHAR(50) NULL AFTER role;

-- Copy dữ liệu role cũ sang role_new và map sang role mới
UPDATE users SET role_new = CASE
    WHEN role = 'admin' THEN 'admin'
    WHEN role = 'manager' THEN 'editor'  -- Map manager -> editor
    WHEN role = 'editor' THEN 'author'   -- Map editor cũ -> author
    WHEN role = 'user' THEN 'reader'     -- Map user -> reader
    ELSE 'reader'  -- Default
END;
...
-- Đổi tên role_new thành role
ALTER TABLE users CHANGE COLUMN role_new role ENUM('admin', 'editor', 'author', 'reader') DEFAULT 'reader';
...
('posts.publish', 'Xuất bản bài viết', 'Xuất bản/Hủy xuất bản', 'posts', 'publish'),
...
('users.manage_roles', 'Quản lý vai trò', 'Gán và thay đổi vai trò', 'users', 'manage'),
-- Comments
...
('comments.moderate', 'Kiểm duyệt bình luận', 'Duyệt/Từ chối', 'comments', 'moderate'),
-- Categories
...
('settings.update', 'Sửa cài đặt', 'Thay đổi cài đặt', 'settings', 'update'),
-- Analytics
...
('editor', 'posts.create'), ('editor', 'posts.read'), ('editor', 'posts.update'),
('editor', 'posts.delete'), ('editor', 'posts.publish'), ('editor', 'posts.manage_all'),
('editor', 'comments.read'), ('editor', 'comments.update'), ('editor', 'comments.delete'),
('editor', 'comments.moderate'), ('editor', 'categories.create'), ('editor', 'categories.read'),
('editor', 'categories.update'), ('editor', 'categories.delete'), ('editor', 'media.upload'),
('editor', 'media.read'), ('editor', 'media.delete'), ('editor', 'media.manage_all'),
('editor', 'analytics.read'), ('editor', 'dashboard.editor')
ON DUPLICATE KEY UPDATE granted_at = CURRENT_TIMESTAMP;

-- Author permissions
INSERT INTO role_permissions (role_name, permission_name) VALUES
('author', 'posts.create'), ('author', 'posts.read'), ('author', 'posts.update'),
('author', 'posts.delete'), ('author', 'comments.create'), ('author', 'comments.read'),
('author', 'comments.update'), ('author', 'comments.delete'), ('author', 'categories.read'),
('author', 'media.upload'), ('author', 'media.read'), ('author', 'media.delete'),
('author', 'analytics.read'), ('author', 'dashboard.author')
ON DUPLICATE KEY UPDATE granted_at = CURRENT_TIMESTAMP;

-- Reader permissions
INSERT INTO role_permissions (role_name, permission_name) VALUES
('reader', 'posts.read'), ('reader', 'comments.create'), ('reader', 'comments.read'),
('reader', 'comments.update'), ('reader', 'categories.read'), ('reader', 'media.read')
ON DUPLICATE KEY UPDATE granted_at = CURRENT_TIMESTAMP;
...
SELECT 
    ...
    r.role_display_name, r.level as role_level,
    r.color as role_color, r.icon as role_icon,
    u.status, u.email_verified, u.last_login,
...
SELECT DISTINCT
    ...
    p.module, p.action, 'role' as source
FROM users u
...
SELECT DISTINCT
    u.id as user_id, u.name as user_name, u.role,
    p.permission_name, p.permission_display_name,
    p.module, p.action, 'user' as source
FROM users u
